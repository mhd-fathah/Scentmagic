...
function init() {
    ...
    this.deg = function () {
      return (
        ((Math.atan((this.y2 - this.y1) / (this.x2 - this.x1)) +
          (((this.x2 - this.x1) / Math.abs(this.x2 - this.x1)) * Math.PI) / 2) *
          180) /
        Math.PI
      );
    };
  ...
  function mouseMoveHandler(e) {
    ...
    var offset = document
      ...
      .getBoundingClientRect().height; // Vertical offset to prevent the weapon to move over the topbar
    var newX, newY;
  ...
  function mouseDownHandler(e) {
    ...
    TweenLite.to(slipperBody, 0.05, {
      x: slipperDx,
      y: -slipperDy,
      ease: Power2.easeIn,
    });
    TweenLite.to(slipperShadow, 0.05, {
      x: -slipperDx,
      y: slipperDy,
      ease: Power3.easeIn,
    });
  ...
  function mouseUpHandler() {
    ...
    TweenLite.to(slipperBody, 0.1, {
      x: 0,
      y: 0,
      ease: Power2.easeIn,
    });
    TweenLite.to(slipperShadow, 0.1, {
      x: 0,
      y: 0,
      ease: Power3.easeIn,
    });
  ...
  function checkHit(e) {
    if (bugs) {
      ...
      bugs.forEach(function (b) {
        ...
        if (
          bugX > mouseX - slipperWidth / 2 &&
          bugX < mouseX + slipperWidth / 2 &&
          bugY > mouseY - slipperHeight / 2 &&
          bugY < mouseY + slipperHeight / 2
        ) {
  ...
  function updateCounter() {
    ...
    if (counter == 0) {
      ...
      showModal(
        "Pheeew! That was exhausting! Thanks to your precious help, we got rid of some nasty bugs. We'll continue working, but in the meantime you can try a refresh!",
        "ok"
      );
    }

    function animateCounter() {
      TweenLite.to(counterContainer, 0.2, {
        scale: 1.5,
        ease: Power2.easeIn,
      });
      TweenLite.to(counterContainer, 0.2, {
  ...
  function showModal(message, cta) {
    ...
    modal.innerHTML =
      "<p>" +
      message +
      "</p><button onclick='closeModal(event)'>" +
      cta +
      "</button>";
